...
BEGIN
    ...
    SELECT
        ...
        COALESCE(s.faturamento, 0)::numeric AS total_faturamento,
        CASE
            WHEN c."dataCadastro"::date >= (NOW() - INTERVAL '90 days') THEN 'novo'
            WHEN c."ultimaCompra"::date < (NOW() - INTERVAL '45 days') THEN 'inativo'
            ELSE 'ativo'
...
BEGIN
    ...
    END IF;
    RETURN QUERY
        SELECT nome FROM data_detailed WHERE nome IS NOT NULL AND (p_supervisor IS NULL OR data_detailed.superv = p_supervisor)
        UNION
        SELECT nome FROM data_history WHERE nome IS NOT NULL AND (p_supervisor IS NULL OR data_history.superv = p_supervisor)
        ORDER BY 1;
END;
$$;
...
BEGIN
    IF NOT public.is_caller_approved() THEN
        RAISE EXCEPTION 'Acesso não autorizado';
    END IF;
    RETURN QUERY SELECT DISTINCT pd.codfor::text, pd.fornecedor::text FROM public.data_product_details pd WHERE pd.fornecedor IS NOT NULL ORDER BY pd.fornecedor;
END;
$$;
...
BEGIN
    ...
    SELECT
        ...
        (EXTRACT(YEAR FROM age(date_trunc('month', now()), date_trunc('month', h.dtped))) * 12 + EXTRACT(MONTH FROM age(date_trunc('month', now()), date_trunc('month', h.dtped))))::integer as mes_historico,
        h.superv,
        h.codcli,
    ...
    GROUP BY h.superv, h.codcli, mes_historico;
END;
$$;
...
BEGIN
    ...
    WITH StockData AS (
        ...
        FROM data_stock s
        WHERE (p_filial = 'ambas' OR s.filial = p_filial)
        GROUP BY s.product_code
    )
    SELECT
        p.descricao::text as produto_descricao,
        p.fornecedor::text,
        COALESCE(sd.total_stock, 0)::numeric as estoque_atual_cx,
        (SELECT AVG(v.total_venda) FROM (SELECT SUM(h.qtvenda) as total_venda FROM data_history h WHERE h.produto = p.code GROUP BY date_trunc('month', h.dtped)) v)::numeric as venda_media_mensal_cx,
        (SELECT SUM(d.qtvenda) / 21.0 FROM data_detailed d WHERE d.produto = p.code)::numeric as media_diaria_cx,
        (COALESCE(sd.total_stock, 0) / NULLIF((SELECT SUM(d.qtvenda) / 21.0 FROM data_detailed d WHERE d.produto = p.code), 0))::numeric as tendencia_dias,
    ...
    LEFT JOIN StockData sd ON p.code = sd.product_code
...
BEGIN
    ...
    SELECT
        p.descricao::text as produto_descricao,
        COALESCE(sd.total_stock, 0)::numeric as estoque_cx,
