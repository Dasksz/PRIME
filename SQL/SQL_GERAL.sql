...
-- ==============================================================================
-- 6. MIGRATION: ADD ROW_HASH FOR DELTA SYNC
-- ==============================================================================
-- Adds a 'row_hash' column to main data tables to enable client-side diffing and incremental uploads.

-- 1. Sales Data
ALTER TABLE public.data_detailed ADD COLUMN IF NOT EXISTS row_hash TEXT;
CREATE INDEX IF NOT EXISTS idx_data_detailed_row_hash ON public.data_detailed (row_hash);

-- 2. History Data
ALTER TABLE public.data_history ADD COLUMN IF NOT EXISTS row_hash TEXT;
CREATE INDEX IF NOT EXISTS idx_data_history_row_hash ON public.data_history (row_hash);

-- 3. Clients Data
ALTER TABLE public.data_clients ADD COLUMN IF NOT EXISTS row_hash TEXT;
CREATE INDEX IF NOT EXISTS idx_data_clients_row_hash ON public.data_clients (row_hash);

-- 4. Stock Data
ALTER TABLE public.data_stock ADD COLUMN IF NOT EXISTS row_hash TEXT;
CREATE INDEX IF NOT EXISTS idx_data_stock_row_hash ON public.data_stock (row_hash);

-- 5. Innovations Data
ALTER TABLE public.data_innovations ADD COLUMN IF NOT EXISTS row_hash TEXT;
CREATE INDEX IF NOT EXISTS idx_data_innovations_row_hash ON public.data_innovations (row_hash);

-- 6. Active Products (Support Table)
ALTER TABLE public.data_active_products ADD COLUMN IF NOT EXISTS row_hash TEXT;
CREATE INDEX IF NOT EXISTS idx_data_active_products_row_hash ON public.data_active_products (row_hash);
