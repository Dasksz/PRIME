...
-- Grant Access for Dimensions
GRANT SELECT ON public.dim_supervisores TO authenticated;
GRANT SELECT ON public.dim_vendedores TO authenticated;
GRANT SELECT ON public.dim_fornecedores TO authenticated;
GRANT SELECT ON public.dim_produtos TO authenticated;

GRANT INSERT, UPDATE, DELETE ON public.dim_supervisores TO authenticated;
GRANT INSERT, UPDATE, DELETE ON public.dim_vendedores TO authenticated;
GRANT INSERT, UPDATE, DELETE ON public.dim_fornecedores TO authenticated;
...
-- ==============================================================================
-- 6. CACHING & PERFORMANCE FUNCTIONS
-- ==============================================================================

-- 6.1 REFRESH CACHE SUMMARY (JOIN-Based Optimization)
create or replace function public.refresh_cache_summary() RETURNS void as $$
BEGIN
  ...
  ANALYZE public.dim_supervisores;
  ANALYZE public.dim_vendedores;
  ANALYZE public.dim_fornecedores;
  ANALYZE public.dim_produtos;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER
set search_path = public;

-- 6.2 GET DETAILED DATA WITH JOINS
-- Used by the frontend to fetch the full dataset efficiently
create or replace function public.get_detailed_data_optimized()
RETURNS TABLE (
    ped text,
    codcli text,
    dtped bigint, -- Using BIGINT for timestamp
    codusur text,
    codsupervisor text,
    produto text,
    qtvenda numeric,
    ...
    superv text,    -- Joined
    nome text,      -- Joined
    descricao text, -- Joined
    fornecedor text -- Joined
) as $$
BEGIN
    RETURN QUERY
    SELECT
        ...
        d.tipovenda,
        d.filial,
        COALESCE(s.superv, 'N/A') as superv,
        COALESCE(v.nome, 'N/A') as nome,
        COALESCE(p.descricao, 'N/A') as descricao,
        COALESCE(f.fornecedor, 'N/A') as fornecedor
    FROM public.data_detailed d
    LEFT JOIN public.dim_supervisores s ON d.codsupervisor = s.codsupervisor
    LEFT JOIN public.dim_vendedores v ON d.codusur = v.codusur
    LEFT JOIN public.dim_produtos p ON d.produto = p.produto
    LEFT JOIN public.dim_fornecedores f ON p.codfor = f.codfor;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER
set search_path = public;

-- 6.3 GET HISTORY DATA WITH JOINS
create or replace function public.get_history_data_optimized()
RETURNS TABLE (
    ped text,
    codcli text,
    dtped bigint,
...
BEGIN
    ...
    LEFT JOIN public.dim_supervisores s ON h.codsupervisor = s.codsupervisor
    LEFT JOIN public.dim_vendedores v ON h.codusur = v.codusur
    LEFT JOIN public.dim_produtos p ON h.produto = p.produto
    LEFT JOIN public.dim_fornecedores f ON p.codfor = f.codfor;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER
set search_path = public;
